    ...
    public abstract class InstantiationInfo
    {
        ...
        private static void CheckTargetType(Type targetType, TypeInfo targetTypeInfo)
        {
            ...
            if (targetTypeInfo.IsAbstract)
                throw new TypeRegistrationException($"You cannot register type {targetTypeInfo} because it is an abstract class which cannot be instantiated. Only non-abstract types that are either non-generic, closed generic, or generic type definitions are allowed.");
