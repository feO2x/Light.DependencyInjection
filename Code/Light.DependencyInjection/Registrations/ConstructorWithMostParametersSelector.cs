    ...
    public sealed class ConstructorWithMostParametersSelector : IDefaultInstantiationInfoSelector
    {
        ...
        private static void CheckConstructorGroup(IEnumerable<ConstructorInfo> constructorGroup, TypeInfo typeInfo)
        {
            ...
            if (constructorGroup == null)
                throw new RegistrationException($"Cannot register \"{type}\" with the DI container because this type does not contain a public non-static constructor. Please specify an instantiation method using the registration options.");

            if (constructorGroup.Count() == 1)
                return;

            throw new RegistrationException($"Cannot register \"{type}\" with the DI container because this type contains two or more constructors with the same number of parameters. Please specify an instantiation method using the registration options.");
        }
    }
}