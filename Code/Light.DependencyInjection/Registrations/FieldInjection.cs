    ...
    public sealed class FieldInjection : InstanceManipulation
    {
            ...
            : base(typeKey, targetField.MustNotBeNull(nameof(targetField)).Name, dependencies)
        {
            ...
            if (foundField == null || foundField.Equals(targetField) == false)
                throw new TypeRegistrationException($"The type \"{TypeKey.Type}\" does not contain the field \"{targetField}\".", typeKey.Type);

            if (foundField.IsStatic)
                throw new TypeRegistrationException($"The specified field \"{targetField}\" is a static field and thus cannot be used for Field Injection.", TypeKey.Type);
