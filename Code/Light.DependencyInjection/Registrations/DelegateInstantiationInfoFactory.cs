    ...
    public sealed class DelegateInstantiationInfoFactory : InstantiationInfoFactory
    {
        ...
        public DelegateInstantiationInfoFactory(Type targetType, Delegate @delegate)
            : base(targetType.MustNotBeNull(nameof(targetType)),
                   @delegate.MustNotBeNull(nameof(@delegate)).GetMethodInfo().ExtractDependencies(targetType))
        {
            Delegate = @delegate;
        }
        ...
        {
            return new DelegateInstantiationInfo(new TypeKey(TargetType, registrationName), Delegate, InstantiationDependencyFactories.CreateInstantiationDependencies(registrationName));
        }
    }
}